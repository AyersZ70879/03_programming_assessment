...
class Start:
    def __init__(self, parent):
        ...
        # Quiz Heading (row 0)
        self.quiz_box_label = Label(self.start_frame, text="Country - Capital Game", font="Arial 19 bold", bg="#DDF0FF")
    ...
    def check_rounds(self):
        ...
        if has_errors == "yes":
            self.start_amounts_entry.config(bg=error_back)
            self.amount_error_label.config(text=error_feedback)

        else:
            # set starting balance to amount entered by user
            self.starting_funds.set(starting_rounds)
            ...
            "You will be played {} rounds.".format(starting_rounds),
                                           ...
                                           justify=LEFT, padx=10, pady=10)
            self.quiz_rounds_accept.grid(row=2)
